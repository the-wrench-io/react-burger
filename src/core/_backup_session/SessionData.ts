...
class ImmutableTab<T> implements API.SessionTab<T> {
  private _id: string;
...
class SessionData implements API.Session {
  private _appId: string;
  private _tabs: ImmutableTab<any>[];
  private _history: API.SessionHistory;
  ...
  get tabs(): readonly API.SessionTab<any>[] {
  ...
  private next(history: API.SessionHistory, tabs?: ImmutableTab<any>[]): API.Session {
  ...
  withTabData(tabId: string, updateCommand: (oldData: any) => any): API.Session {
  ...
  withTab(newTabOrTabIndex: API.SessionTab<any> | number): API.Session {
    ...
    const newTab = new ImmutableTab<any>(newTabOrTabIndex as API.SessionTab<any>);
  ...
  withSecondary(newItemId?: string): API.Session {
    return new SessionData({ 
      secondary: newItemId, 
      tabs: this._tabs, 
  ...
  withDrawer(open: boolean): API.Session {
    return new SessionData({ 
      secondary: this._secondary, 
      tabs: this._tabs, 
      history: this._history, 
  ...
  withAppId(appId: string): API.Session {
    return new SessionData({ 
      secondary: this._secondary, 
      tabs: this._tabs, 
      history: this._history, 
  ...
  deleteTab(tabId: string): API.Session {
    const tabs: ImmutableTab<any>[] = [];
    for (const tab of this._tabs) {
      if (tabId !== tab.id) {
        tabs.push(tab);
